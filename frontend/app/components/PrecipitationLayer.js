...
const PrecipitationLayer = ({ map, period, onValueChange }) => {
    ...
    const loadPrecipitationData = async () => {
      try {
        ...
        for (let i = 0; i < values.length; i++) {
          ...
          // Si el punto está fuera de Coquimbo o el valor es 0, hacerlo transparente
          if (!isPointInCoquimbo(lat, lng) || value === 0) {
            imageData.data[i * 4 + 3] = 0; // alpha = 0
            transparentPixels++;
            continue;
          }

          // Para valores dentro de Coquimbo y mayores a 0
  ...
  const getColorForValue = (value) => {
    ...
    return {
      r: Math.floor(intensity * 0.5), // Azul con un toque de rojo para profundidad
      g: Math.floor(intensity * 0.7), // Azul con un toque de verde para brillo
      b: intensity // Componente azul principal
    };
  };
